...
## D8: Install Impeccable Once, Globally — Not Per Project

**Decision:** Install the `impeccable` CLI/skill pack ([impeccable.style](https://impeccable.style)) **once**, as a personal (global) skill at `~/.claude/skills/impeccable/` — not re-installed per project via `--scope=project`. A global personal skill loads automatically in every project's Claude Code session, so there is no reason to duplicate the ~2.2MB skill payload into each repo's `.claude/skills/`.

Originally (see the superseded text below) this was installed per-project. In practice `npx impeccable skills install --scope=global` did not reliably honor the scope flag (kept writing to the cwd's `.claude/` regardless of `-y`/`--scope`/`--force` combinations tried) — the reliable fix was to let it install once, then `mv` the resulting folder from the project into `~/.claude/skills/impeccable` directly and delete the project-local copy. If reinstalling/updating later, expect to repeat that manual move rather than trusting the CLI's scope flag.

After a global install (or the manual move), **restart the Claude Code session** (skills added to `~/.claude/skills/` aren't picked up by an already-running harness — `/impeccable init` will fail with "Unknown command" until then), then run `/impeccable init` per project to generate that project's PRODUCT.md (and DESIGN.md when relevant) design context — `init` is still a per-project step even though the skill install itself is not.

**Why:** Impeccable provides 45 deterministic anti-pattern detectors (accessibility/contrast violations, AI-generated-UI "tells" like gradient text and side-tab accents, typography and layout quality issues) plus a structured command set (craft, audit, critique, polish, brand, colorize, harden, etc.) that give every project a consistent, tool-enforced design quality bar instead of relying on ad-hoc judgment calls each session. It's built specifically for AI coding agents (Claude, Cursor, Codex, Gemini), so the commands and reference docs are already tuned for exactly this kind of workflow. Installing it globally instead of per-project avoids re-downloading and re-committing the same third-party payload into every repo, and keeps it available even in repos where no one remembered to install it.

**Tradeoff accepted:** A global skill applies to every project on the machine, including ones where design-quality tooling wasn't explicitly requested — acceptable since the detectors are read-only/advisory until a command is invoked. `/impeccable init` (which generates project-specific PRODUCT.md/DESIGN.md) remains a manual per-project step. The mid-session-install-needs-a-restart gotcha still applies, but now it's a one-time cost per machine instead of per project.
